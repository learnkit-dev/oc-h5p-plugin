...
class EditorStorage implements \H5peditorStorage
{
    ...
    public function getLibraries($libraries = null)
    {
        ...
        if ($libraries !== null) {
            ...
            foreach ($libraries as $library) {
                // Look for library
                $details = Library::where('name', $library->name)
            ...
            // Load all libraries
            $libraries = [];
            $libraries_result = Library::where('runnable', 1)
                ->select([
                    //                        'id',
                    'name',
                    'title',
            ...
            foreach ($libraries_result as $library) {
                $library['uberName'] = $library['name'] . ' ' . $library['majorVersion'] . '.' . $library['minorVersion'];
    ...
    public static function markFileForCleanup($file, $content_id)
    {
        ...
        Tmpfile::create(['path' => $path, 'created_at' => time()]);
