...
class LaravelH5pRepository implements \H5PFrameworkInterface
{
    ...
    {
        $where = Library::select()->where('name', $name);
    ...
    public function getLibraryUsage($id, $skipContent = false)
    {
        ...
        return [
            'content'   => $content,
            'libraries' => intval(LibrariesLibrary::where('required_library_id', $id)->count()),
        ];
    }
    ...
    public function updateContent($entry, $contentMainId = null)
    {
        ...
        if (!isset($entry['id'])) {
            ...
            Content::where('id', $content['id'])->update($content);
            $event_type = 'update';
        }
    ...
    public function deleteContentData($id)
    {
        ...
        Result::where('content_id', $id)->delete();
        ContentsUserData::where('content_id', $id)->delete();
    }
    ...
    {
        ContentsLibrary::where('content_id', $contentId)->delete();
    }
    ...
    public function loadLibrarySemantics($name, $majorVersion, $minorVersion)
    {
        if ($this->isInDevMode()) {
            ...
            $semantics = Library::where('name', $name)
                ->where('major_version', $majorVersion)
                ->where('minor_version', $minorVersion)
                ->first();
    ...
    {
        Content::where('library_id', $library_id)->update(['filtered' => null]);
    }
    ...
    {
        return Content::where('filtered', '')->count();
    }
    ...
    {
        return Content::where('library_id', $library_id)->count();
    }
    ...
    public function getPlatformInfo()
    {
        return [
            'name'       => 'October CMS',
            'version'    => Parameter::get('system:core.build'),
